...
/*  Date:     21-SEP-2017                            */
/*
 Contributors: Huzaifa Asif -- 100921147 & Ali Selcuk - 100954050
 (c) 2017 Louis Nel                               */
...
void MyTunes::executeCMDSHOW(Command cmd){
    ...
    collectionName = cmd.getToken(COLLECTION);
    view.setClassName(collectionName);  //...
    
    
    if(cmd.isTokenAt(COLLECTION, "songs")){
        if (!cmd.hasToken("-s")){
            songs.showOn(view);
        }
        else{
            songs.showOn(view, cmd.getToken("-s"));
        }
    }
    else if (cmd.isTokenAt(COLLECTION, "recordings")){
        if (!cmd.hasToken("-r")){
            recordings.showOn(view);
        }
        else {
            recordings.showOn(view, cmd.getToken("-r"));
        }
    }
    else if (cmd.isTokenAt(COLLECTION, "tracks")){
        if (!cmd.hasToken("-t")){
            tracks.showOn(view);
        }
        else {
            tracks.showOn(view, cmd.getToken("-t"));
        }
    }
    else if (cmd.isTokenAt(COLLECTION, "users")){
        if (!cmd.hasToken("-u")){
            users.showOn(view);
        }
        else{
...
void MyTunes::executeAddTrack(Command cmd){
    ...
    if(cmd.getToken(RECORDING_ID).compare("null") == 0) recording = NULL;
        ...
        recording = recordings.genericSearch(cmd.getToken(RECORDING_ID));
        
    }
    
    int track_number = stoi(cmd.getToken(TRACK_NUMBER));
    Track* track = new Track(id, songs.genericSearch(cmd.getToken(SONG_ID)));
...
void MyTunes::executeAddPlaylist(Command cmd){
    ...
    User* user = users.genericSearch(cmd.getToken(USERID));
    if(user == NULL) return;
...
void MyTunes::executeAddPlaylistTrack(Command cmd){
    ...
    if(playlist == NULL) return;
    Track * track = tracks.genericSearch(cmd.getToken(TRACK_ID));
    if(track == NULL) return;
...
void MyTunes::executeDeleteRecording(Command cmd){
    ...
    Recording* recording = recordings.genericSearch(cmd.getToken(ID));
    if(recording == NULL) return;
...
void MyTunes::executeDeleteUser(Command cmd){
    ...
    User* user = users.genericSearch(cmd.getToken(USER_ID));
    if(user == NULL) return;
    users.remove(*user);
...
void MyTunes::executeDeleteUserPlaylist(Command cmd){
    ...
    User* user = users.genericSearch(user_id);
    if(user == NULL) return;
...
void MyTunes::executeDeleteUserPlaylistTrack(Command cmd){
    ...
    Playlist * playlist = user->findPlaylist(playlistName);
    if(playlist == NULL) return;
    Track * track = tracks.genericSearch(trackIDstring);
    if(track == NULL) return;
...
void MyTunes::executeDeleteSong(Command cmd){
    ...
    Song* song = songs.genericSearch(cmd.getToken(ID));
    if(song == NULL) return;
